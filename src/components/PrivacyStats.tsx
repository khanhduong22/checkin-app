...
export default function PrivacyStats({
    totalHours,
    totalSalary,
    daysWorked,
    baseSalary,
            ...
            <div className="rounded-lg bg-secondary/50 p-3 relative group cursor-pointer" onClick={() => setIsVisible(!isVisible)}>
                ...
                {isVisible && (baseSalary !== undefined) && (
                    <div className="text-[10px] text-muted-foreground mt-1 space-y-0.5 border-t pt-1 border-dashed">
                        ...
                        {latePenaltyHours !== undefined && latePenaltyHours > 0 && (
                            <div className="flex justify-between font-medium text-red-500">
                                <span>🕐 Phạt đi trễ ({latePenaltyHours}h):</span>
                                <span>− {f(latePenaltyAmount ?? 0)}</span>
                            </div>
                        )}
                    </div>
                )}
