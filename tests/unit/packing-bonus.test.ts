...
describe("runPackingBonus()", () => {
  ...
  it("awards +100k to the top user with points <= 50 on the last day of the month past 18:00", async () => {
    ...
    // User task data: User 1 has 9 points, User 2 has 12 points
    ...
    expect(mockCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          userId: "user-2",
          amount: 100000,
          reason: "Thưởng Vua Đóng Hàng (Top 1 T5/2026)",
        }),
      })
    );
  });

  it("awards +200k to the top user with points > 50 on the last day of the month past 18:00", async () => {
    vi.setSystemTime(new Date("2026-05-31T19:00:00+07:00"));
  ...
  it("does NOT award bonus if there are no approved packing tasks", async () => {
    vi.setSystemTime(new Date("2026-05-31T19:00:00+07:00"));
  ...
  it("does NOT award bonus if top user has 0 points", async () => {
    vi.setSystemTime(new Date("2026-05-31T19:00:00+07:00"));
  ...
  it("gracefully catches database errors without crashing", async () => {
    ...
    await expect(runPackingBonus()).resolves.not.toThrow();
  });
});
